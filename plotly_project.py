# -*- coding: utf-8 -*-
"""Plotly project.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dpMbgkVg5mJQ-e2TIjU-JsGfMxcFLdLx
"""
...
df1 = pd.read_csv("/content/zomato.csv")
df2 = pd.read_csv("/content/Country-Code.xlsx - Sheet1.csv")


import requests
Conv_list = []

for i in df2["Currency Code"]:
...
app.layout = html.Div([
    ...
    html.H1("ZOMATO DASHBORD",className="bg-primary text-center text-white"),

html.Div([
        html.Label("Country-Specific Data : ", className="fs-5 fw-bold"),
        dcc.Dropdown(
    id="dropdown",
...
html.Div([
        dbc.Row([
         dbc.Col(dcc.Graph(id="Totalcost-chart"),width=6,className="rounded"),
        dbc.Col(dcc.Graph(id="Totalrating-chart"),width=6,className="rounded")
        ])
])

     ] ),
html.Div([
        html.Label("Select City to Analyze :",className="fs-5 fw-bold"),
        dcc.Dropdown(
            id="city_dropdown",
            value="Bangalore",className="bg-opacity-50 p-2 m-1 bg-primary text-dark fw-bold rounded",
    style={'width':"50%"}

        ), 
        html.Div([
             dbc.Row([
        dbc.Col(dcc.Graph(id="City_fav_cus-chart"),width=6,className="rounded"),
        dbc.Col(dcc.Graph(id="City_costly_cus-chart"),width=6,className="rounded")
...
html.Br(),
 html.Div([
     dbc.Row([
        dbc.Col(dcc.Graph(id="City_rating-chart"))
  ])
 ]),
...
html.Div([
dbc.Row([
        dbc.Col(dcc.Graph(id="City_delivery-chart"),width=6,className="rounded"),
       dbc.Col(dcc.Graph(id="din-in-chart"),width=6,className="rounded")
         
])]),

html.Div([
        html.Label("Select City to compare :",className="fs-5 fw-bold"),
        dcc.Dropdown(
            id="citydropdown_1",
            value="Bangalore",
            multi=True,
...
dbc.Row([
        ...
        dbc.Col(dcc.Graph(id="multi-chart_1"),width=4,className="rounded"),
        dbc.Col(dcc.Graph(id="multi-chart_2"),width=4,className="rounded"),
        dbc.Col(dcc.Graph(id="multi-chart_3"),width=4,className="rounded")
])])
]),
html.P("Designed by Bhaarani V G ", className="float-end text-dark")
...
def update_bar_chart(Country):
   ...
   fig = px.bar(city_cost, y= "Amount Spent",color_discrete_sequence=[ "goldenrod"])
...
def update_comp_chart_1(val_chosen):
  ...
  fig = px.bar(a, y="Total Amount Spent",color_discrete_sequence=[ "goldenrod"])
